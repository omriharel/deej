...
void buttonPressActions(){    
  ...
  if(buttonState[1]==0 && oldButtonState[1] == 1){
    ...
    Keyboard.println("\"C:\\Zip Programs\\Sound Volume\\SoundVolumeView.exe\" /Switch \"{0.0.1.00000000}.{55d36775-205b-4934-a4a3-2541e604e287}\""); //types path to deej run
    delay(100);
...
int knobRead(int knob){ //smooth knob read
  ...
  for (int i = 0; i<avgSize; i++){ //for avgSize iterations
    int kread = analogRead(knob);
    if (knob == A3){
      kread = 1024 - kread; //I got one knob backwards, oh well.
    }
    analogAverage = kread + analogAverage; //take analog value
    delayMicroseconds(int(10000/avgSize)); //every millisecond
  }
